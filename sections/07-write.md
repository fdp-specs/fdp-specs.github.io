...
A profile ([[#metadata-records]]) hosted by the FDP, and the metadata schema it refers to as its validation artifact, are resources that are created, replaced and deleted with the same operations as metadata records, with the profile description or the SHACL shapes graph as the request body.
The FDP *MUST* validate that a submitted profile description conforms to [[#metadata-records]] and that a submitted schema is a syntactically valid SHACL shapes graph that declares its target class, and *MUST* reject the deletion of a profile or schema that is still referenced, with `409 Conflict`.
