...
These specifications define how applications should behave in order to be classified as a FAIR Data Point. FDP uses a HATEOAS REST API, associated with Linked Data Platform for serving FAIR metadata. FDP is software that, from one side, allows digital objects owners/publishers to expose the metadata of their digital objects in a FAIR manner and, from the other side, allows digital objects' consumers to discover information (metadata) about offered digital objects.
...
Many different repositories and their digital objects should interoperate in order to allow increasingly complex questions to be answered. These repositories and their content should be interoperable in order for client applications to autonomously (re)use them.
...
- A common interface to access information (metadata) about digital objects (HATEOAS REST APIs with [[LDP]]);
...
The main goal of the FDP is to provide a common mechanism for applications to expose metadata about themselves and about their content and how these metadata should be encoded and structured. This provide to client applications a predictable way of accessing and interacting with metadata content, improving interoperability. To fulfill this goal, this document contains a set of specifications to help developers to implement the FDP functionality in their new or existing applications.
